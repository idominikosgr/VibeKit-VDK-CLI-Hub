...
async function getAdminUserId(): Promise<string> {
  ...
  const { data: adminUser } = await supabase
    .from('profiles')
    .select('id')
    .eq('email', 'dominikos@myroomieapp.com')
    .single();
  ...
  // Check for any existing profile
  ...
  const { data: systemUser, error } = await supabase
    ...
    .insert({
      ...
      email: 'system@codepilotrules.com',
      name: 'System'
    })
    .select()
    .single();
...
async function createAgenticAITag() {
  ...
  const { data: tag, error } = await supabase
    ...
    .insert({
      name: 'agentic-ai',
      slug: 'agentic-ai',
      description: 'Agentic artificial intelligence concepts and development',
      color: '#9333EA'
    })
    .select()
    .single();
...
async function createAgenticAIPage() {
  ...
  const tagId = await createAgenticAITag();
...
The Vibe Coding Rules Hub serves as both a repository of knowledge and a practical platform for developing and deploying agentic AI systems in real-world development environments.`;
  ...
  if (!existingAssociation) {
    ...
    if (tagError) {
      console.error('Warning: Failed to associate tag:', tagError.message);
    } else {
      console.log('✅ Tag associated successfully');
    }
  }
...
async function main() {
  try {
    ...
    const pageId = await createAgenticAIPage();
    
    console.log('✅ Successfully created/updated Agentic AI Development page!');
