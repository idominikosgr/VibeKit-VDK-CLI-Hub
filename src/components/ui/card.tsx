...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-background text-foreground flex flex-col gap-6 rounded-lg border border-border p-6 shadow-sm transition-all duration-normal hover:shadow-md hover:border-primary/50 hover:-translate-y-1",
        className
      )}
      {...props}
    />
