...
export default function ProfilePage() {
  ...
  const handleDownloadPackage = async (pkg: GeneratedPackage) => {
    ...
    try {
      ...
      a.href = url;
      a.download = `rules-package-${pkg.package_type}-${pkg.id.slice(0, 8)}.${
        pkg.package_type === "zip" ? "zip" : "sh"
      }`;
                  ...
                  {packagesLoading ? (
                    <div className="space-y-4">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="h-20 w-full bg-gradient-to-r from-muted via-muted/60 to-muted rounded-lg animate-pulse"
                        />
                      ))}
                    </div>
                  ) : packages.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground mb-4">
                        Generate your first package using the VibeKit Rule
                        Generator
                      </p>
                      <Button asChild>
                        <Link href="/generate">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="space-y-2">
                              ...
                              {pkg.download_count && pkg.download_count > 0 && (
                                <div className="text-sm text-muted-foreground">
                                  Downloaded {pkg.download_count}{" "}
                                  {pkg.download_count === 1 ? "time" : "times"}
                                </div>
                              )}
                              {pkg.expires_at && (
                                <div className="text-sm text-muted-foreground">
