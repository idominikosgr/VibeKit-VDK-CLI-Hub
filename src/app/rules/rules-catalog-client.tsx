...
import {
  fetchRuleCategories,
  fetchMagnifyingGlassRules,
  fetchAllRules,
  fetchSingleRule,
} from "@/lib/services/client-rule-service";
...
export function RulesCatalogClient() {
  ...
  const [categoriesView, setCategoriesView] = useState<"cards" | "details">(
    "cards"
  );
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (tab === "rules") {
          if (searchQuery) {
            const rulesResult = await fetchMagnifyingGlassRules(
              searchQuery,
              1,
              50
            );
      ...
      >
        <div className="flex-1 space-y-4">
          ...
          <p className="text-xl text-muted-foreground">
            Browse our comprehensive collection of AI-assisted development rules
            and guidelines.
          </p>
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-4">
            <div className="w-full max-w-sm">
              <MagnifyingGlassBar
                placeholder={
                  tab === "rules" ? "Search rules..." : "Search categories..."
                }
              />
            </div>
            <ViewToggle
              ...
              {loading ? (
                ...
                >
                  {categories.map((category, index) => (
                    ...
                    >
                      <CategoryCard category={category} view={categoriesView} />
                    </motion.div>
                  ))}
                </motion.div>
              ) : (
