...
import { getRulesByCategory, searchRules, findRuleByIdentifier } from "@/lib/services/supabase-rule-service";
...
export function SetupPreview({ config }: { config: SetupFormValues | null }) {
  ...
  const generateRulePreview = async () => {
    if (!config) return "// Select categories and options to preview rule content"
    ...
    try {
      ...
      if (config.primaryLanguage) {
        const langKey = config.primaryLanguage === "other" ?
      ...
      if (config.aiAssistant) {
        ...
        const assistantKey = config.aiAssistant === "other" ?
...
compatibility:
  ...
  frameworks: [${rule.compatibility?.frameworks?.map((fw: string) => `"${fw}"`).join(', ') || ''}]
---
...
${rule.content}`;
  ...
  const [rulePreview, setRulePreview] = useState<string>("// Loading rule preview...");
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="preview" className="space-y-4">
          <Card>
            ...
            <CardFooter>
              <Button onClick={handleGeneratePackage} disabled={!config || isGenerating}>
                {isGenerating ? (
                  <>Generating Package</>
                ) : (
                  <>Generate Package</>
              ...
              <div className="space-y-4 rounded-md border p-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>
                      {config?.primaryLanguage === "other" ? config?.otherLanguage : config?.primaryLanguage}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">IDE:</span>
                    <span>{config?.ide === "other" ? config?.otherIde : config?.ide}</span>
                  </div>
                </div>
              </div>
              <Separator />
